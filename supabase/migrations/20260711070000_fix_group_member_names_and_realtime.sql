create or replace function public.get_space_members(user_id_input uuid default auth.uid())
returns table (
  id uuid,
  nickname text,
  emoji_identity text,
  avatar_key text,
  theme_preference public.theme_preference,
  bio text,
...
as $$
  ...
  inner join public.profiles p
    on p.active_couple_id = viewer.active_couple_id
  where viewer.id = coalesce(user_id_input, auth.uid())
    and viewer.active_couple_id is not null
  order by p.created_at asc;
$$;

drop policy if exists "profiles_select_self_or_partner" on public.profiles;
drop policy if exists "profiles_select_self_or_space_members" on public.profiles;
create policy "profiles_select_self_or_space_members"
  on public.profiles
  for select
  to authenticated
  using (
